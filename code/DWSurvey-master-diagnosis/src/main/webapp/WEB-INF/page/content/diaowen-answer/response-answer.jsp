				...
				<form action="${ctx }/sy/user/nosm/survey-answer!save"
					...
					<div class="middle-body-content">
						<c:forEach items="${questions }" var="en" varStatus="i">
							...
							<c:if
								test="${en.quType ne 'PARAGRAPH' && en.quType ne 'PAGETAG' }">

								<%--题干 --%>
								<div class="r-qu-body">
									<div class="r-qu-body-title">
										<span class="quCoNum">${i.count }</span>、${en.quTitle }[${en.quType.cnName }]
									</div>
									<div class="r-qu-body-content">
										<c:choose>
													...
													<c:choose>
														<c:when test="${en.anYesno.yesnoAnswer eq 1 }">
															<li><input type="radio"
																...
																value="${en.yesnoOption.trueValue }" checked="checked" />${en.yesnoOption.trueValue }</li>
															<li><input type="radio"
																name="qu_${en.quType }_${en.id }"
																value="${en.yesnoOption.falseValue }" />${en.yesnoOption.falseValue }
															</li>
														</c:when>
														<c:when test="${en.anYesno.yesnoAnswer eq 0 }">
															<li><input type="radio"
																name="qu_${en.quType }_${en.id }"
																value="${en.yesnoOption.trueValue }" />${en.yesnoOption.trueValue }</li>
															<li><input type="radio"
																name="qu_${en.quType }_${en.id }"
																value="${en.yesnoOption.falseValue }" checked="checked" />${en.yesnoOption.falseValue }
															</li>
														</c:when>
														<c:otherwise>
															<li><input type="radio"
															...
															<ul class="quItem-ul">
																<c:choose>
																	<c:when test="${en.anRadio.quItemId eq quItem.id }">
																		<li><input type="radio"
																			name="qu_${en.quType }_${en.id }"
																			value="${quItem.id }" checked="checked" /></li>
														...
														<c:when test="${fn:length(en.anCheckboxs) gt 0 }">
															<c:forEach items="${en.quCheckboxs }" var="quItem">
																<li><ul class="quItem-ul">
																		...
																		<c:forEach items="${en.anCheckboxs }" var="anCk"
																			varStatus="iStatus">
																			<c:choose>
																				<c:when test="${anCk.quItemId eq quItem.id }">
																					<li><input type="checkbox"
																						name="tag_qu_${en.quType }_${en.id }_${quItem.id }"
																						value="${quItem.id }" checked="checked" /></li>
																					<li>${quItem.optionName }</li>
																					<c:set var="isBreak" value="1"></c:set>
																				</c:when>
																				<c:when
																					test="${(fn:length(en.anCheckboxs) eq iStatus.count ) && (isBreak eq 0) }">
																					<li><input type="checkbox"
																						name="tag_qu_${en.quType }_${en.id }_${quItem.id }"
																						value="${quItem.id }" /></li>
																					<li>${quItem.optionName }</li>
											...
											<c:when test="${en.quType eq 'ANSWER' }">
												<ul class="r-qu-body-ul1 r-qu-body-ul3">
													<li><textarea rows="5" cols="50"
															name="qu_${en.quType }_${en.id }">${en.anAnswer.answer }</textarea>
													</li>
												</ul>
											</c:when>
											<%--复合单选 --%>
													...
													<c:forEach items="${en.quRadios }" var="quItem">
														<li>
															<ul class="quItem-ul ">
																<c:choose>
																	<c:when test="${en.anRadio.quItemId eq quItem.id }">
																		<li class="${quItem.isNote eq 1 ?'quItem-ul01li':''}"><input
																			type="radio" name="qu_${en.quType }_${en.id }"
																			value="${quItem.id }" checked="checked" /></li>
																		<li>${quItem.optionName }<c:if
																				test="${quItem.isNote eq 1 }">&nbsp;&nbsp; 说明：<input
																					type="text"
																					name="text_qu_${en.quType }_${en.id }_${quItem.id }"
																					value="${en.anRadio.otherText }" />
																			</c:if></li>
																	</c:when>
																	<c:otherwise>
																		<li class="${quItem.isNote eq 1 ?'quItem-ul01li':''}"><input
																...
																<li><ul class="quItem-ul">
																		...
																		<c:forEach items="${en.anCheckboxs }" var="anCk"
																			...
																			<c:choose>
																				<c:when test="${anCk.quItemId eq quItem.id }">
																					<li
																						class="${quItem.isNote eq 1 ?'quItem-ul01li':''}"><input
																						type="checkbox"
																						name="tag_qu_${en.quType }_${en.id }_${quItem.id}"
																						value="${quItem.id }" checked="checked" /></li>
																					<li>${quItem.optionName }<c:if
													...
													<c:choose>
														<c:when test="${fn:length(en.anEnumqus) gt 0 }">
															<c:forEach begin="1" end="${en.paramInt01 }" var="quItem">
																<c:set var="isBreak" value="0"></c:set>
																<c:forEach items="${en.anEnumqus }" var="anEn"
																	varStatus="iStatus">
																	<c:choose>
																		<c:when test="${anEn.enumItem eq quItem }">
																			<li>${quItem }、<input type="text"
																				name="qu_${en.quType }_${en.id }_${quItem}"
																				value="${anEn.answer }" /></li>
																			<c:set var="isBreak" value="1"></c:set>
																		</c:when>
																		<c:when
																			test="${(fn:length(en.anEnumqus) eq iStatus.count ) && (isBreak eq 0)}">
																			<li>${quItem }、<input type="text"
																				name="qu_${en.quType }_${en.id }_${quItem}" /></li>
																			<c:set var="isBreak" value="0"></c:set>
																		</c:when>
																	</c:choose>
																</c:forEach>
													...
													<c:choose>
														<c:when test="${fn:length(en.anDFillblanks) gt 0 }">
															<c:forEach items="${en.quMultiFillblanks }" var="quItem">
																<c:set var="isBreak" value="0"></c:set>
																<c:forEach items="${en.anDFillblanks }" var="anDf"
																	varStatus="iStatus">
																	<c:choose>
																		<c:when test="${anDf.quItemId eq quItem.id }">
																			<li>${quItem.optionName }<input type="text"
																				name="text_qu_${en.quType }_${en.id }_${quItem.id }"
																				value="${anDf.answer }" /></li>
																			<c:set var="isBreak" value="1"></c:set>
																		</c:when>
																		<c:when
																			test="${(fn:length(en.anDFillblanks) eq iStatus.count ) && (isBreak eq 0)}">
																			<li>${quItem.optionName }<input type="text"
																				name="text_qu_${en.quType }_${en.id }_${quItem.id }"
																				value="" /></li>
																			<c:set var="isBreak" value="0"></c:set>
											...
											<c:when test="${en.quType eq 'SCORE' }">
												...
												<table class="qusTable" width="90%">
													<tr>
														<th width="80">行/分</th>
														<c:forEach begin="${en.paramInt01 }"
															end="${en.paramInt02 }" var="enCol">
														...
														<tr>
															...
															<c:forEach begin="${en.paramInt01 }"
																...
																<td align="center"><input type="radio"
																	name="item_qu_${en.quType }_${en.id }_${quScore.id }"
																	value="${enCol }" /></td>
															</c:forEach>
														</tr>
													</c:forEach>
												...
												<c:forEach items="${en.anScores }" var="enAn">
													<script type="text/javascript">
														$(
																"input[name='item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }'][value='${enAn.answserScore }']")
																.attr(
																		"checked",
																		true);
													</script>
											...
											<c:when test="${en.quType eq 'CHENRADIO' }">
												...
												<table class="qusTable" width="90%">
													...
													<c:forEach items="${en.rows }" var="enRow">
														<tr>
															...
															<c:forEach items="${en.columns }" var="enCol">
																<td align="center"><input type="radio"
																	name="item_qu_${en.quType }_${en.id }_${enRow.id }"
																	value="${enCol.id }" /></td>
															</c:forEach>
														</tr>
													</c:forEach>
												...
												<c:forEach items="${en.anChenRadios }" var="enAn">
													<script type="text/javascript">
														$(
																"input[name='item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }'][value='${enAn.quColId }']")
																.attr(
																		"checked",
																		true);
													</script>
											...
											<c:when test="${en.quType eq 'CHENCHECKBOX' }">
												...
												<table class="qusTable" width="90%">
													...
													<c:forEach items="${en.rows }" var="enRow">
														<tr>
															<td align="center">${enRow.optionName }<input
																type="hidden"
																name="item_qu_${en.quType }_${en.id }_${enRow.id }"
																value="ck_item_qu_${en.quType }_${en.id }_${enRow.id }" />
															</td>
															<c:forEach items="${en.columns }" var="enCol">
																<td align="center"><input type="checkbox"
																	name="ck_item_qu_${en.quType }_${en.id }_${enRow.id }_${enCol.id}"
																	value="${enCol.id }" /></td>
															</c:forEach>
														</tr>
													</c:forEach>
												...
												<c:forEach items="${en.anChenCheckboxs }" var="enAn">
													<script type="text/javascript">
														$(
																"input[name='ck_item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }_${enAn.quColId }'][value='${enAn.quColId }']")
																.attr(
																		"checked",
																		true);
													</script>
											...
											<c:when test="${en.quType eq 'CHENFBK' }">
													...
													<c:forEach items="${en.rows }" var="enRow">
														<tr>
															<td align="center">${enRow.optionName }<input
																type="hidden"
																name="item_qu_${en.quType }_${en.id }_${enRow.id }"
																value="fbk_item_qu_${en.quType }_${en.id }_${enRow.id }" /></td>
															<c:forEach items="${en.columns }" var="enCol">
																<td align="center"><input type="text"
																	name="fbk_item_qu_${en.quType }_${en.id }_${enRow.id }_${enCol.id }"
																	size="6" /></td>
															</c:forEach>
														</tr>
													</c:forEach>
												...
												<c:forEach items="${en.anChenFbks }" var="enAn">
													<script type="text/javascript">
														$(
																"input[name='fbk_item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }_${enAn.quColId }']")
																.val(
																		"${enAn.answerValue}");
													</script>
												</c:forEach>
											...
											<c:when test="${en.quType eq 'COMPCHENRADIO' }">
													...
													<c:forEach items="${en.rows }" var="enRow">
														<tr>
															<td align="center">${enRow.optionName }<input
																type="hidden"
																name="item_qu_${en.quType }_${en.id }_${enRow.id}"
																value="sel_item_qu_${en.quType }_${en.id }_${enRow.id}" /></td>
															<c:forEach items="${en.columns }" var="enCol">
																<td align="center"><select
																	name="sel_item_qu_${en.quType }_${en.id }_${enRow.id}_${enCol.id}">
																		<option>-请选择-</option>
																		<c:forEach items="${en.options }" var="enOption">
																			<option value="${enOption.id }">${enOption.optionName }
																			</option>
																		</c:forEach>
																</select></td>
															</c:forEach>
												...
												<c:forEach items="${en.anCompChenRadios }" var="enAn">
													<script type="text/javascript">
														$(
																"select[name='sel_item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }_${enAn.quColId }']")
																.val(
																		"${enAn.quOptionId}");
													</script>
												</c:forEach>
