...
import com.key.dwsurvey.service.impl.EParametersManagerImpl;
 ...
 *
 *         https://github.com/wkeyuan/DWSurvey http://dwsurvey.net
 *
 */
...
public class ResponseAction extends ActionSupport {
	...
	private String e_uid;// 外部唯一
	private String e_desc;// 描述文字
	@Autowired
	...
	public String answerMobile() throws Exception {
		...
		if (directory != null) {
			...
			System.out.println("/" + htmlPath + "/m_" + surveyId + ".html");
	...
	public String save() throws Exception {
		...
		HttpServletResponse response = Struts2Utils.getResponse();
		try {
			// 如果e_uid 不为空的话，要判断是否重复提交
			if (e_uid != null && !"".equals(e_uid)) {
				// 判断系统里面，是否已经重复提交了
				boolean isOverrideSubmit = eParametersManager.checkIsExist(this.e_uid, this.surveyId);
				if (isOverrideSubmit) {
					request.setAttribute("surveyName", "此问卷已经回答，不允许重复提交");
					request.setAttribute("msg", "此问卷已经回答，不允许重复提交");
					return RESPONSE_MSG;
			...
			// 保存额外参数
			saveExtendParameter(entity.getId());
	...
	public String saveMobile() throws Exception {
		...
		try {
			...
			if (e_uid != null && !"".equals(e_uid)) {
				// 判断系统里面，是否已经重复提交了
				boolean isOverrideSubmit = eParametersManager.checkIsExist(this.e_uid.trim(), this.surveyId);
				if (isOverrideSubmit) {
					request.setAttribute("surveyName", "此问卷已经回答，不允许重复提交");
					request.setAttribute("msg", "此问卷已经回答，不允许重复提交");
					return RESPONSE_MSG;
	...
	public Map<String, Map<String, Object>> buildSaveSurveyMap(HttpServletRequest request) {
		...
		Map<String, Object> dfillblankMaps = WebUtils.getParametersStartingWith(request,
				"qu_" + QuType.MULTIFILLBLANK + "_");// 多项填空
		...
		Map<String, Object> chenCheckboxMaps = WebUtils.getParametersStartingWith(request,
				"qu_" + QuType.CHENCHECKBOX + "_");
		...
		Map<String, Object> chenCompChenRadioMaps = WebUtils.getParametersStartingWith(request,
				"qu_" + QuType.COMPCHENRADIO + "_");
	...
	public String answerSuccessM() throws Exception {
		System.err.println(surveyId);
		SurveyDirectory directory = null;
		HttpServletRequest request = Struts2Utils.getRequest();
		try {
			if (surveyId == null||surveyId.equals("")) {
				directory = new SurveyDirectory();
			} else {
				directory = directoryManager.get(surveyId);
			}
	...
	private void saveExtendParameter(String answerID) {
		if (e_uid == null || "".equals(e_uid)) {
			return;
		}
		EParameters entityParamter = new EParameters();
		entityParamter.setBelongAnswerId(answerID);
		entityParamter.setBelongId(surveyId);
		entityParamter.seteDesc(e_desc);
		entityParamter.setVisibility(1);
		if (e_uid != null) {
			e_uid = e_uid.trim();
			entityParamter.seteUid(e_uid);
			eParametersManager.save(entityParamter);
		}
	}
